...
//#region node_modules/gantt-task-react/dist/index.modern.js
...
var addToDate = function addToDate(date, quantity, scale) {
	return new Date(date.getFullYear() + (scale === "year" ? quantity : 0), date.getMonth() + (scale === "month" ? quantity : 0), date.getDate() + (scale === "day" ? quantity : 0), date.getHours() + (scale === "hour" ? quantity : 0), date.getMinutes() + (scale === "minute" ? quantity : 0), date.getSeconds() + (scale === "second" ? quantity : 0), date.getMilliseconds() + (scale === "millisecond" ? quantity : 0));
};
var startOfDate = function startOfDate(date, scale) {
	...
	return new Date(date.getFullYear(), shouldReset("year") ? 0 : date.getMonth(), shouldReset("month") ? 1 : date.getDate(), shouldReset("day") ? 0 : date.getHours(), shouldReset("hour") ? 0 : date.getMinutes(), shouldReset("minute") ? 0 : date.getSeconds(), shouldReset("second") ? 0 : date.getMilliseconds());
};
...
var getLocaleMonth = function getLocaleMonth(date, locale) {
	var bottomValue = getCachedDateTimeFormat(locale, { month: "long" }).format(date);
	bottomValue = bottomValue.replace(bottomValue[0], bottomValue[0].toLocaleUpperCase());
	return bottomValue;
};
var getLocalDayOfWeek = function getLocalDayOfWeek(date, locale, format) {
	var bottomValue = getCachedDateTimeFormat(locale, { weekday: format }).format(date);
	bottomValue = bottomValue.replace(bottomValue[0], bottomValue[0].toLocaleUpperCase());
	return bottomValue;
};
...
var getWeekNumberISO8601 = function getWeekNumberISO8601(date) {
	...
	if (tmpDate.getDay() !== 4) tmpDate.setMonth(0, 1 + (4 - tmpDate.getDay() + 7) % 7);
...
var TaskListHeaderDefault = function TaskListHeaderDefault(_ref) {
	...
	return import_react.createElement("div", {
	...
	}, "\xA0Name"), import_react.createElement("div", {
		...
		className: styles.ganttTable_HeaderItem,
		style: { minWidth: rowWidth }
	}, "\xA0From"), import_react.createElement("div", {
		className: styles.ganttTable_HeaderSeparator,
		style: {
			height: headerHeight * .5,
			marginTop: headerHeight * .25
		}
	}), import_react.createElement("div", {
		className: styles.ganttTable_HeaderItem,
		style: { minWidth: rowWidth }
...
var TaskListTableDefault = function TaskListTableDefault(_ref) {
	...
	return import_react.createElement("div", {
		...
		return import_react.createElement("div", {
			...
			title: t.name
		}, import_react.createElement("div", { className: styles$1.taskListNameWrapper }, import_react.createElement("div", {
			className: expanderSymbol ? styles$1.taskListExpander : styles$1.taskListEmptyExpander,
			onClick: function onClick() {
				return onExpanderClick(t);
			}
		}, expanderSymbol), import_react.createElement("div", null, t.name))), import_react.createElement("div", {
			className: styles$1.taskListCell,
			style: {
				minWidth: rowWidth,
				maxWidth: rowWidth
			}
		}, "\xA0", toLocaleDateString(t.start, dateTimeOptions)), import_react.createElement("div", {
			className: styles$1.taskListCell,
			style: {
				minWidth: rowWidth,
				maxWidth: rowWidth
...
var Tooltip = function Tooltip(_ref) {
	...
	(0, import_react.useEffect)(function() {
		if (tooltipRef.current) {
			...
			if (rtl) {
				...
				if (newRelatedX < 0) newRelatedX = task.x2 + arrowIndent * 1.5 - scrollX;
				if (tooltipWidth + newRelatedX > svgContainerWidth) {
					newRelatedX = svgContainerWidth - tooltipWidth;
					newRelatedY += rowHeight;
				}
			} else {
				newRelatedX = task.x2 + arrowIndent * 1.5 + taskListWidth - scrollX;
				if (tooltipWidth + newRelatedX > taskListWidth + svgContainerWidth) newRelatedX = task.x1 + taskListWidth - arrowIndent * 1.5 - scrollX - tooltipWidth;
				if (newRelatedX < taskListWidth) {
					newRelatedX = svgContainerWidth + taskListWidth - tooltipWidth;
					newRelatedY += rowHeight;
				}
			}
			if (tooltipHeight + newRelatedY - scrollY > svgContainerHeight - scrollY) newRelatedY = svgContainerHeight - tooltipHeight;
	...
	}, import_react.createElement(TooltipContent, {
		task,
		fontSize,
		fontFamily
	}));
...
var StandardTooltipContent = function StandardTooltipContent(_ref2) {
	var task = _ref2.task, fontSize = _ref2.fontSize;
	var style = {
		fontSize,
	...
	return import_react.createElement("div", {
		...
		style
	}, import_react.createElement("b", { style: { fontSize: fontSize + 6 } }, task.name + ": " + task.start.getDate() + "-" + (task.start.getMonth() + 1) + "-" + task.start.getFullYear() + " - " + task.end.getDate() + "-" + (task.end.getMonth() + 1) + "-" + task.end.getFullYear()), task.end.getTime() - task.start.getTime() !== 0 && import_react.createElement("p", { className: styles$2.tooltipDefaultContainerParagraph }, "Duration: " + ~~((task.end.getTime() - task.start.getTime()) / (1e3 * 60 * 60 * 24)) + " day(s)"), import_react.createElement("p", { className: styles$2.tooltipDefaultContainerParagraph }, !!task.progress && "Progress: " + task.progress + " %"));
};
...
var TaskList = function TaskList(_ref) {
	...
	return import_react.createElement("div", { ref: taskListRef }, import_react.createElement(TaskListHeader, Object.assign({}, headerProps)), import_react.createElement("div", {
		ref: horizontalContainerRef,
		className: horizontalContainerClass,
		style: ganttHeight ? { height: ganttHeight } : {}
	}, import_react.createElement(TaskListTable, Object.assign({}, tableProps))));
};
var styles$4 = {
	"gridRow": "_2dZTy",
	"gridRowLine": "_3rUKi",
...
var GridBody = function GridBody(_ref) {
	...
	var y = 0;
	var gridRows = [];
	var rowLines = [import_react.createElement("line", {
		key: "RowLineFirst",
		x: "0",
		y1: 0,
		x2: svgWidth,
	...
	for (var _iterator = _createForOfIteratorHelperLoose(tasks), _step; !(_step = _iterator()).done;) {
		var task = _step.value;
		gridRows.push(import_react.createElement("rect", {
			key: "Row" + task.id,
			x: "0",
			y,
			width: svgWidth,
		...
		rowLines.push(import_react.createElement("line", {
			key: "RowLine" + task.id,
			x: "0",
			y1: y + rowHeight,
			x2: svgWidth,
	...
	var tickX = 0;
	var ticks = [];
	var today = import_react.createElement("rect", null);
	for (var i = 0; i < dates.length; i++) {
		var date = dates[i];
		ticks.push(import_react.createElement("line", {
			key: date.getTime(),
			x1: tickX,
			y1: 0,
			x2: tickX,
		...
		if (i + 1 !== dates.length && date.getTime() < now.getTime() && dates[i + 1].getTime() >= now.getTime() || i !== 0 && i + 1 === dates.length && date.getTime() < now.getTime() && addToDate(date, date.getTime() - dates[i - 1].getTime(), "millisecond").getTime() >= now.getTime()) today = import_react.createElement("rect", {
			x: tickX,
			y: 0,
			width: columnWidth,
			height: y,
		...
		if (rtl && i + 1 !== dates.length && date.getTime() >= now.getTime() && dates[i + 1].getTime() < now.getTime()) today = import_react.createElement("rect", {
			x: tickX + columnWidth,
			y: 0,
			width: columnWidth,
			height: y,
	...
	return import_react.createElement("g", { className: "gridBody" }, import_react.createElement("g", { className: "rows" }, gridRows), import_react.createElement("g", { className: "rowLines" }, rowLines), import_react.createElement("g", { className: "ticks" }, ticks), import_react.createElement("g", { className: "today" }, today));
};
var Grid = function Grid(props) {
	return import_react.createElement("g", { className: "grid" }, import_react.createElement(GridBody, Object.assign({}, props)));
};
var styles$5 = {
...
var TopPartOfCalendar = function TopPartOfCalendar(_ref) {
	...
	return import_react.createElement("g", { className: "calendarTop" }, import_react.createElement("line", {
		x1: x1Line,
		y1: y1Line,
		x2: x1Line,
		y2: y2Line,
...
var Calendar = function Calendar(_ref) {
	...
	var getCalendarValuesForYear = function getCalendarValuesForYear() {
		var topValues = [];
		var bottomValues = [];
		var topDefaultHeight = headerHeight * .5;
		for (var i = 0; i < dateSetup.dates.length; i++) {
			var date = dateSetup.dates[i];
			var bottomValue = date.getFullYear();
			bottomValues.push(import_react.createElement("text", {
				key: date.getFullYear(),
				y: headerHeight * .8,
				x: columnWidth * i + columnWidth * .5,
				className: styles$5.calendarBottomText
			}, bottomValue));
			if (i === 0 || date.getFullYear() !== dateSetup.dates[i - 1].getFullYear()) {
				var topValue = date.getFullYear().toString();
				var xText = void 0;
				if (rtl) xText = (6 + i + date.getFullYear() + 1) * columnWidth;
				else xText = (6 + i - date.getFullYear()) * columnWidth;
				topValues.push(import_react.createElement(TopPartOfCalendar, {
					key: topValue,
					value: topValue,
	...
	var getCalendarValuesForMonth = function getCalendarValuesForMonth() {
		...
		for (var i = 0; i < dateSetup.dates.length; i++) {
			...
			if (i === 0 || date.getFullYear() !== dateSetup.dates[i - 1].getFullYear()) {
				var topValue = date.getFullYear().toString();
				var xText = void 0;
				if (rtl) xText = (6 + i + date.getMonth() + 1) * columnWidth;
				else xText = (6 + i - date.getMonth()) * columnWidth;
				topValues.push(import_react.createElement(TopPartOfCalendar, {
					key: topValue,
					value: topValue,
					x1Line: columnWidth * i,
	...
	var getCalendarValuesForWeek = function getCalendarValuesForWeek() {
		...
		for (var i = dates.length - 1; i >= 0; i--) {
			var date = dates[i];
			var topValue = "";
			if (i === 0 || date.getMonth() !== dates[i - 1].getMonth()) topValue = getLocaleMonth(date, locale) + ", " + date.getFullYear();
			...
			if (topValue) {
				if (i !== dates.length - 1) topValues.push(import_react.createElement(TopPartOfCalendar, {
					key: topValue,
					value: topValue,
					x1Line: columnWidth * i + weeksCount * columnWidth,
					y1Line: 0,
					y2Line: topDefaultHeight,
					xText: columnWidth * i + columnWidth * weeksCount * .5,
					yText: topDefaultHeight * .9
				}));
				weeksCount = 0;
			}
		...
		for (var i = 0; i < dates.length; i++) {
			...
			if (i + 1 !== dates.length && date.getMonth() !== dates[i + 1].getMonth()) {
				...
				topValues.push(import_react.createElement(TopPartOfCalendar, {
					...
					y1Line: 0,
					y2Line: topDefaultHeight,
					xText: columnWidth * (i + 1) - getDaysInMonth(date.getMonth(), date.getFullYear()) * columnWidth * .5,
					yText: topDefaultHeight * .9
				}));
			}
		}
	...
	var getCalendarValuesForPartOfDay = function getCalendarValuesForPartOfDay() {
		...
		for (var i = 0; i < dates.length; i++) {
			var date = dates[i];
			var bottomValue = getCachedDateTimeFormat(locale, { hour: "numeric" }).format(date);
			bottomValues.push(import_react.createElement("text", {
				key: date.getTime(),
				y: headerHeight * .8,
				x: columnWidth * (i + +rtl),
			...
			if (i === 0 || date.getDate() !== dates[i - 1].getDate()) {
				...
				topValues.push(import_react.createElement(TopPartOfCalendar, {
					...
					y1Line: 0,
					y2Line: topDefaultHeight,
					xText: columnWidth * i + ticks * columnWidth * .5,
					yText: topDefaultHeight * .9
				}));
			}
		}
...
/*#__PURE__*/ typeof Symbol !== "undefined" && (Symbol.iterator || (Symbol.iterator = Symbol("Symbol.iterator")));
...
var Arrow = function Arrow(_ref) {
	...
	return import_react.createElement("g", { className: "arrow" }, import_react.createElement("path", {
		strokeWidth: "1.5",
		d: path,
		fill: "none"
	}), import_react.createElement("polygon", { points: trianglePoints }));
};
...
var drownPathAndTriangleRTL = function drownPathAndTriangleRTL(taskFrom, taskTo, rowHeight, taskHeight, arrowIndent) {
	...
	return ["M " + taskFrom.x1 + " " + (taskFrom.y + taskHeight / 2) + " \n  h " + -arrowIndent + " \n  v " + indexCompare * rowHeight / 2 + " \n  " + taskFromHorizontalOffsetValue + "\n  V " + taskToEndPosition + " \n  h " + taskToHorizontalOffsetValue, taskTo.x2 + "," + taskToEndPosition + " \n  " + (taskTo.x2 + 5) + "," + (taskToEndPosition + 5) + " \n  " + (taskTo.x2 + 5) + "," + (taskToEndPosition - 5)];
};
var convertToBarTasks = function convertToBarTasks(tasks, dates, columnWidth, rowHeight, taskHeight, barCornerRadius, handleWidth, rtl, barProgressColor, barProgressSelectedColor, barBackgroundColor, barBackgroundSelectedColor, projectProgressColor, projectProgressSelectedColor, projectBackgroundColor, projectBackgroundSelectedColor, milestoneBackgroundColor, milestoneBackgroundSelectedColor) {
	...
	barTasks = barTasks.map(function(task) {
		...
		for (var j = 0; j < dependencies.length; j++) _loop(j);
		return task;
	});
	return barTasks;
};
...
var convertToMilestone = function convertToMilestone(task, index, dates, columnWidth, rowHeight, taskHeight, barCornerRadius, handleWidth, milestoneBackgroundColor, milestoneBackgroundSelectedColor) {
	...
	var y = taskYCoordinate(index, rowHeight, taskHeight);
	var x1 = x - taskHeight * .5;
	var x2 = x + taskHeight * .5;
...
var taskXCoordinate = function taskXCoordinate(xDate, dates, columnWidth) {
	...
	var percentOfInterval = (xDate.getTime() - dates[index].getTime()) / (dates[index + 1].getTime() - dates[index].getTime());
...
var progressWithByParams = function progressWithByParams(taskX1, taskX2, progress, rtl) {
	var progressWidth = (taskX2 - taskX1) * progress * .01;
	var progressX;
	if (rtl) progressX = taskX2 - progressWidth;
	else progressX = taskX1;
...
var startByX = function startByX(x, xStep, task) {
	if (x >= task.x2 - task.handleWidth * 2) x = task.x2 - task.handleWidth * 2;
	var additionalXValue = Math.round((x - task.x1) / xStep) * xStep;
	return task.x1 + additionalXValue;
};
var endByX = function endByX(x, xStep, task) {
	if (x <= task.x1 + task.handleWidth * 2) x = task.x1 + task.handleWidth * 2;
	var additionalXValue = Math.round((x - task.x2) / xStep) * xStep;
	return task.x2 + additionalXValue;
};
var moveByX = function moveByX(x, xStep, task) {
	var additionalXValue = Math.round((x - task.x1) / xStep) * xStep;
	var newX1 = task.x1 + additionalXValue;
	return [newX1, newX1 + task.x2 - task.x1];
};
...
var handleTaskBySVGMouseEventForBar = function handleTaskBySVGMouseEventForBar(svgX, action, selectedTask, xStep, timeStep, initEventX1Delta, rtl) {
	...
	switch (action) {
		case "progress":
			if (rtl) changedTask.progress = progressByXRTL(svgX, selectedTask);
			else changedTask.progress = progressByX(svgX, selectedTask);
		...
		case "start":
			...
			if (isChanged) {
				if (rtl) changedTask.end = dateByX(newX1, selectedTask.x1, selectedTask.end, xStep, timeStep);
				else changedTask.start = dateByX(newX1, selectedTask.x1, selectedTask.start, xStep, timeStep);
		...
		case "end":
			...
			if (isChanged) {
				if (rtl) changedTask.start = dateByX(newX2, selectedTask.x2, selectedTask.start, xStep, timeStep);
				else changedTask.end = dateByX(newX2, selectedTask.x2, selectedTask.end, xStep, timeStep);
...
function removeHiddenTasks(tasks) {
	...
	if (groupedTasks.length > 0) {
		...
		for (var i = 0; groupedTasks.length > i; i++) _loop(i);
	}
	return tasks;
}
function getChildren(taskList, task) {
	var tasks = [];
	if (task.type !== "project") tasks = taskList.filter(function(t) {
		return t.dependencies && t.dependencies.indexOf(task.id) !== -1;
	});
	else tasks = taskList.filter(function(t) {
...
var BarDisplay = function BarDisplay(_ref) {
	...
	return import_react.createElement("g", { onMouseDown }, import_react.createElement("rect", {
		x,
		width,
		y,
		height,
...
var BarProgressHandle = function BarProgressHandle(_ref) {
	...
	return import_react.createElement("polygon", {
		className: styles$6.barHandle,
...
var Bar = function Bar(_ref) {
	...
	return import_react.createElement("g", {
		...
		tabIndex: 0
	}, import_react.createElement(BarDisplay, {
		x: task.x1,
		y: task.y,
	...
	}), import_react.createElement("g", { className: "handleGroup" }, isDateChangeable && import_react.createElement("g", null, import_react.createElement(BarDateHandle, {
		x: task.x1 + 1,
		y: task.y + 1,
	...
	}), import_react.createElement(BarDateHandle, {
	...
	})), isProgressChangeable && import_react.createElement(BarProgressHandle, {
		progressPoint,
		onMouseDown: function onMouseDown(e) {
...
var BarSmall = function BarSmall(_ref) {
	...
	}), import_react.createElement("g", { className: "handleGroup" }, isProgressChangeable && import_react.createElement(BarProgressHandle, {
		progressPoint,
		onMouseDown: function onMouseDown(e) {
			onEventStart("progress", task, e);
		}
...
var Milestone = function Milestone(_ref) {
	...
	var transform = "rotate(45 " + (task.x1 + task.height * .356) + " \n    " + (task.y + task.height * .85) + ")";
	return import_react.createElement("g", {
		tabIndex: 0,
...
var TaskItem = function TaskItem(props) {
	...
	var _useState = (0, import_react.useState)(import_react.createElement("div", null)), taskItem = _useState[0], setTaskItem = _useState[1];
	var _useState2 = (0, import_react.useState)(true), isTextInside = _useState2[0], setIsTextInside = _useState2[1];
	(0, import_react.useEffect)(function() {
		switch (task.typeInternal) {
			case "milestone":
				setTaskItem(import_react.createElement(Milestone, Object.assign({}, props)));
				break;
			case "project":
				setTaskItem(import_react.createElement(Project, Object.assign({}, props)));
				break;
			case "smalltask":
				setTaskItem(import_react.createElement(BarSmall, Object.assign({}, props)));
				break;
			default:
				setTaskItem(import_react.createElement(Bar, Object.assign({}, props)));
				break;
		}
	}, [task, isSelected]);
	(0, import_react.useEffect)(function() {
		if (textRef.current) setIsTextInside(textRef.current.getBBox().width < task.x2 - task.x1);
	}, [textRef, task]);
	return import_react.createElement("g", {
	...
	}, taskItem, import_react.createElement("text", {
		x: function getX() {
			var width = task.x2 - task.x1;
			var hasChild = task.barChildren.length > 0;
			if (isTextInside) return task.x1 + width * .5;
			if (rtl && textRef.current) return task.x1 - textRef.current.getBBox().width - arrowIndent * +hasChild - arrowIndent * .2;
			else return task.x1 + width + arrowIndent * +hasChild + arrowIndent * .2;
		}(),
		y: task.y + taskHeight * .5,
...
var TaskGanttContent = function TaskGanttContent(_ref) {
	...
	(0, import_react.useEffect)(function() {
		...
		setXStep(timeStep * columnWidth / dateDelta);
	}, [
		columnWidth,
		dates,
		timeStep
	...
	(0, import_react.useEffect)(function() {
		var handleMouseMove = function handleMouseMove(event) {
			try {
				...
				var _handleTaskBySVGMouse = handleTaskBySVGMouseEvent(point.matrixTransform(svg === null || svg === void 0 ? void 0 : (_svg$current$getScree = svg.current.getScreenCTM()) === null || _svg$current$getScree === void 0 ? void 0 : _svg$current$getScree.inverse()).x, ganttEvent.action, ganttEvent.changedTask, xStep, timeStep, initEventX1Delta, rtl), isChanged = _handleTaskBySVGMouse.isChanged, changedTask = _handleTaskBySVGMouse.changedTask;
				if (isChanged) setGanttEvent({
		...
		var handleMouseUp = function handleMouseUp(event) {
			try {
				...
				event.preventDefault();
				point.x = event.clientX;
				var newChangedTask = handleTaskBySVGMouseEvent(point.matrixTransform(svg === null || svg === void 0 ? void 0 : (_svg$current$getScree2 = svg.current.getScreenCTM()) === null || _svg$current$getScree2 === void 0 ? void 0 : _svg$current$getScree2.inverse()).x, action, changedTask, xStep, timeStep, initEventX1Delta, rtl).changedTask;
					...
					if ((action === "move" || action === "end" || action === "start") && onDateChange && isNotLikeOriginal) {
						var _temp8 = _catch(function() {
							return Promise.resolve(onDateChange(newChangedTask, newChangedTask.barChildren)).then(function(result) {
								if (result !== void 0) operationSuccess = result;
							});
						}, function() {
			...
			return Promise.resolve(function() {
				if (!event) {
					...
					if (isKeyboardEvent(event)) {
									...
									if (onDelete) {
										var _temp16 = _catch(function() {
											return Promise.resolve(onDelete(task)).then(function(result) {
												if (result !== void 0 && result) setGanttEvent({
													action,
													changedTask: task
												});
											});
						...
						if (!ganttEvent.action) setGanttEvent({
							action,
							changedTask: task,
					...
					} else if (action === "dblclick") onDoubleClick && onDoubleClick(task);
					else if (action === "click") onClick && onClick(task);
					else if (action === "move") {
						var _svg$current$getScree3;
						if (!(svg !== null && svg !== void 0 && svg.current) || !point) return;
						point.x = event.clientX;
						setInitEventX1Delta(point.matrixTransform((_svg$current$getScree3 = svg.current.getScreenCTM()) === null || _svg$current$getScree3 === void 0 ? void 0 : _svg$current$getScree3.inverse()).x - task.x1);
						setGanttEvent({
							action,
							changedTask: task,
							originalSelectedTask: task
	...
	return import_react.createElement("g", { className: "content" }, import_react.createElement("g", {
		className: "arrows",
		fill: arrowColor,
		stroke: arrowColor
	}, tasks.map(function(task) {
		return task.barChildren.map(function(child) {
			return import_react.createElement(Arrow, {
		...
		return import_react.createElement(TaskItem, {
			task,
			arrowIndent,
			taskHeight,
...
var TaskGantt = function TaskGantt(_ref) {
	...
	var newBarProps = _extends({}, barProps, { svg: ganttSVGRef });
	(0, import_react.useEffect)(function() {
		if (horizontalContainerRef.current) horizontalContainerRef.current.scrollTop = scrollY;
	}, [scrollY]);
	(0, import_react.useEffect)(function() {
	...
	return import_react.createElement("div", {
	...
	}, import_react.createElement(Calendar, Object.assign({}, calendarProps))), import_react.createElement("div", {
		ref: horizontalContainerRef,
		...
		fontFamily: barProps.fontFamily,
		ref: ganttSVGRef
	}, import_react.createElement(Grid, Object.assign({}, gridProps)), import_react.createElement(TaskGanttContent, Object.assign({}, newBarProps)))));
};
var styles$a = {
...
var HorizontalScroll = function HorizontalScroll(_ref) {
	...
	return import_react.createElement("div", {
		dir: "ltr",
		style: { margin: rtl ? "0px " + taskListWidth + "px 0px 0px" : "0px 0px 0px " + taskListWidth + "px" },
...
var Gantt = function Gantt(_ref) {
	...
	var _useState7 = (0, import_react.useState)({ action: "" }), ganttEvent = _useState7[0], setGanttEvent = _useState7[1];
	...
	(0, import_react.useEffect)(function() {
		var filteredTasks;
		if (onExpanderClick) filteredTasks = removeHiddenTasks(tasks);
	...
	(0, import_react.useEffect)(function() {
		...
		if (changedTask) {
			if (action === "delete") {
				...
				if (prevStateTask && (prevStateTask.start.getTime() !== changedTask.start.getTime() || prevStateTask.end.getTime() !== changedTask.end.getTime() || prevStateTask.progress !== changedTask.progress)) setBarTasks(barTasks.map(function(t) {
	...
	(0, import_react.useEffect)(function() {
		if (ganttHeight) setSvgContainerHeight(ganttHeight + headerHeight);
		else setSvgContainerHeight(tasks.length * rowHeight + headerHeight);
	}, [
		ganttHeight,
		tasks,
		headerHeight,
	...
	(0, import_react.useEffect)(function() {
		...
		var handleWheel = function handleWheel(event) {
			if (event.shiftKey || event.deltaX) {
				var newScrollX = scrollX + (event.deltaX ? event.deltaX : event.deltaY);
				...
				else if (newScrollY > ganttFullHeight - ganttHeight) newScrollY = ganttFullHeight - ganttHeight;
		...
		(_wrapperRef$current = wrapperRef.current) === null || _wrapperRef$current === void 0 || _wrapperRef$current.addEventListener("wheel", handleWheel, { passive: false });
		return function() {
			var _wrapperRef$current2;
			(_wrapperRef$current2 = wrapperRef.current) === null || _wrapperRef$current2 === void 0 || _wrapperRef$current2.removeEventListener("wheel", handleWheel);
		};
	}, [
		wrapperRef,
		scrollY,
	...
	var handleScrollY = function handleScrollY(event) {
		if (scrollY !== event.currentTarget.scrollTop && !ignoreScrollEvent) {
			...
			setIgnoreScrollEvent(true);
		} else setIgnoreScrollEvent(false);
	};
	...
	var handleSelectedTask = function handleSelectedTask(taskId) {
		...
		if (onSelect) {
			if (oldSelectedTask) onSelect(oldSelectedTask, false);
			if (newSelectedTask) onSelect(newSelectedTask, true);
		}
		setSelectedTask(newSelectedTask);
	};
	var handleExpanderClick = function handleExpanderClick(task) {
		if (onExpanderClick && task.hideChildren !== void 0) onExpanderClick(_extends({}, task, { hideChildren: !task.hideChildren }));
	};
	var gridProps = {
		columnWidth,
		svgWidth,
	...
	return import_react.createElement("div", null, import_react.createElement("div", {
		className: styles$9.wrapper,
		onKeyDown: handleKeyDown,
		tabIndex: 0,
		ref: wrapperRef
	}, listCellWidth && import_react.createElement(TaskList, Object.assign({}, tableProps)), import_react.createElement(TaskGantt, {
		gridProps,
		calendarProps,
		barProps,
		ganttHeight,
		scrollY,
		scrollX
	}), ganttEvent.changedTask && import_react.createElement(Tooltip, {
		arrowIndent,
		rowHeight,
		svgContainerHeight,
		svgContainerWidth,
		...
		rtl,
		svgWidth
	}), import_react.createElement(VerticalScroll, {
		ganttFullHeight,
		ganttHeight,
		headerHeight,
		scroll: scrollY,
		...
		rtl
	})), import_react.createElement(HorizontalScroll, {
		svgWidth,
		taskListWidth,
		scroll: scrollX,
		rtl,
